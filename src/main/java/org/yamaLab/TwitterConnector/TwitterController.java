...
//					new connectTwitterTask().execute("");
	...
	private void loginTwitter(){
        ...
        service.parseCommand("guiMessage","twitter login start");
        try{
        // アクセストークンの設定
...
//          }
        	...
        	System.out.println("twitter login error:"+e.toString());
            service.parseCommand("guiMessage","twitter login error：" + e.toString());
...
//					new connectTwitterTask().execute("");
	...
	String delTailSpace(String x){
		     StringBuffer w=new StringBuffer(x);
		     int lp=w.length()-1;
		     if(lp<0) return x;
		     while(w.charAt(lp)==' '){
		        lp--;
		     }
		     String rtn=w.substring(0,lp+1);
		     return rtn;
	}
	String delSpace(String x){
		String x1=Util.skipSpace(x);
		String x2=delTailSpace(x1);
		return x2;
	}
	...
	private void tweetWhen(String tw, String t){
		...
		String gy=delSpace(st.nextToken());
		String gmon=delSpace(st.nextToken());
		String gday=delSpace(st.nextToken());
		String gh=delSpace(st.nextToken());
		String gmin=delSpace(st.nextToken());
		String gsec=delSpace(st.nextToken());
